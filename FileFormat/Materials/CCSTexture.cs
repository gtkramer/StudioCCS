...
public class CCSTexture : CCSBaseObject
{
    ...
    public override bool Read(System.IO.BinaryReader bStream, int sectionSize)
    {
        ...
        Width = bStream.ReadByte();
        Height = bStream.ReadByte();
        bStream.ReadInt16();
    ...
    */
    public bool InitTexture(int _clutID)
    {
        ...
        */
        ...
        if (TextureType == CCS_TEXTURE_DXT1 || TextureType == CCS_TEXTURE_DXT5)
        {
            ...
            {
                tmpFormat = InternalFormat.CompressedRgbaS3tcDxt5Ext;
            }
    ...
    public bool InitTexture_Gen3()
    {
        ...
        InternalFormat tmpFormat = InternalFormat.CompressedRgbaS3tcDxt1Ext;
        if (TextureType == CCS_TEXTURE_DXT1)
        {
            tmpFormat = InternalFormat.CompressedRgbaS3tcDxt1Ext;
        }
        ...
        if (parentSubFile != null)
        {
            for (int i = 0; i < parentSubFile.ObjectIDs.Count; i++)
            {
                ...
                if (tmpType == CCSFile.SECTION_CLUT)
                {
                    ...
                    if (tmpID == CLUTID)
                    {
                        tmpSubNode.Text += " (Default)";
                    }

                    if (tmpID == CurrentCLUTID)
                    {
                        tmpSubNode.Text += " (Current)";
                    }
