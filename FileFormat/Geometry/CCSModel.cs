...
public class CCSModel : CCSBaseObject
{
    ...
    */
    ...
    public override bool DeInit()
    {
        ...
        {
            SubModel tmpSubModel = SubModels[i];
            if (tmpSubModel.VertexArrayID != -1)
            {
                GL.DeleteVertexArray(tmpSubModel.VertexArrayID);
            }

            tmpSubModel.VertexArrayID = -1;
            if (tmpSubModel.VertexBufferID != -1)
            {
                GL.DeleteBuffer(tmpSubModel.VertexBufferID);
            }

            tmpSubModel.VertexBufferID = -1;
            if (tmpSubModel.ElementArrayID != -1)
            {
                GL.DeleteBuffer(tmpSubModel.ElementArrayID);
            }
    ...
    */
    ...
    public void Render(Matrix4 mtx, int extraOptions = 0, int single = -1)
    {
        if ((ModelType & CCS_MODEL_SHADOW) == 0)
        {
            ...
            if (ProgramID == -1 || SubModelCount == 0)
            {
                return;
            }
            ...
            if (single > -1)
            {
                if (single < SubModelCount)
                {
                    RenderSubModel(mtx, single, extraOptions);
                }
            }
            else
            {
    ...
    private bool ReadRigidSubModel(BinaryReader bStream, SubModel _subModel, int _vertexCount)
    {
        ...
        if ((bStream.BaseStream.Position % 4) == 2)
        {
            bStream.ReadUInt16();
        }
    ...
    public void DumpToSMD(StreamWriter outf, bool withNormals)
    {
        ...
        {
            string texName = "None";
            if (tmpSm.ParentTextureRef != null)
            {
                texName = ParentFile.GetSubObjectName(tmpSm.ParentTextureRef.ObjectID);
            }
            ...
            for (int t = 0; t < tmpSm.TriangleCount; t++)
            {
                ...
                for (int v = 0; v < 3; v++)
                {
                    ...
                    if (withNormals)
                    {
                        normalStr = string.Format("{0} {1} {2}", tmpV.Normal.X, tmpV.Normal.Y, tmpV.Normal.Z);
                    }
